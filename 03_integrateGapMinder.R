...
nn<-merge(countriesAttacks3, gDat, by.x=c('country','year'),by.y=c('country','year'))
nn<-nn[-which(nn$gdpPercap==max(nn$gdpPercap)),]
xyplot(gdpPercap~totAttacks|year,nn,group=continent,pch=18,auto.key=TRUE)


countriesAttacktype <-ddply(dat, .(country, attacktype1), function(x){data.frame(sum(x$nkill,na.rm=TRUE))})
...
xyplot(nkill~gdpPercap, vvv, group=attacktype1,auto.key=TRUE,pch=18,cex=1)
# can see USA cuz of 9/11

deadliest <- tail(arrange(subset(dat, !is.na(nkill)),nkill), n=250)
deadliest2<-subset(deadliest,select=c('year','imonth','iday','attacktype1','nkill','nwound','country'))
names(deadliest2)[7]<-'country'
bb<-merge(deadliest2,subset(gDat, year==2007,select=c('country','gdpPercap')))
xyplot(nkill~gdpPercap, bb, group=attacktype1,auto.key=TRUE,pch=18,cex=1)
...
woundest<-subset(woundest,select=c('year','imonth','iday','attacktype1','nkill','nwound','country','tot'))
woundestG<-merge(woundest,subset(gDat, year==2007,select=c('country','gdpPercap','continent')), by.x='country',by.y='country')
xyplot(tot~gdpPercap, woundestG, group=attacktype1,auto.key=TRUE,pch=18,cex=1)
